...
namespace AnimalBattleRoyale
{
    ...
    public sealed class ThirdPersonAnimalController : MonoBehaviour
    {
        ...
        private const float EagleFlightDrainPerSecond = 11f;
        private const float EagleMinimumTakeoffEnergy = 8f;
        private const float MonkeyVineLeapEnergyCost = 34f;
        ...
        private bool monkeyAirJumpUsed;
        ...
        private void SimulateMovement(Vector3 direction, bool sprint, bool jumpPressed, bool jumpHeld, bool descendHeld)
        {
            ...
            else if (animalType == AnimalType.Monkey && jumpPressed && !grounded && !monkeyAirJumpUsed && TrySpendMobilityEnergy(MonkeyAirJumpEnergyCost))
            {
                monkeyAirJumpUsed = true;
        ...
        {
            if (animalType != AnimalType.Monkey || vine == null || IsHangingVine || !TrySpendMobilityEnergy(MonkeyVineLeapEnergyCost)) return false;
            heldVine = vine;
            UpdateVineHang();
        ...
        public bool TryLaunchToVine(Vector3 vinePosition)
        {
            if (animalType != AnimalType.Monkey || !TrySpendMobilityEnergy(MonkeyVineLeapEnergyCost)) return false;
